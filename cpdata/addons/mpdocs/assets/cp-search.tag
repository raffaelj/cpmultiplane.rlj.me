<cp-search>
    ...
    <script>
        ...
        this.on('mount', function(){
            ...
            // custom: added lang to output
            // see: cockpit/assets/lib/uikit/js/uikit.js (line: 314)
            UIkit.autocomplete(this.refs.autocomplete, {
                source: App.route('/cockpit/search'),
                template: '<ul class="uk-nav uk-nav-autocomplete uk-autocomplete-results">{{~items}}<li data-value="" data-url="{{$item.url}}"><a href="{{$item.url}}"><i class="uk-icon-{{ ($item.icon || "cube") }}"></i>{{#$item.lang}} <span class="uk-text-muted">{{$item.lang}}</span>{{/}} {{$item.title}}</a></li>{{/items}}</ul>'
            });
